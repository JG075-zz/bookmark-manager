...
feature "Password reset" do
  ...
  scenario 'when forgotten password, should be able to reset' do
    visit '/sessions/new'
    click_link 'I forgot my password'
    expect(page).to have_content('Please enter your email address')
  end

  scenario 'enter an email to receive a password' do
    recover_password
  ...
  scenario 'it doesn\'t allow you to use the token after an hour' do
    ...
    Timecop.travel(60 * 60 * 60) do
      visit("/users/reset_password?token=#{user.password}")
  ...
  scenario 'it lets you sign in after password reset' do
    recover_password
    set_password(password: 'newpassword', password_confirmation: 'newpassword')
    sign_in(username: 'alice@example.com', password: 'newpassword')
